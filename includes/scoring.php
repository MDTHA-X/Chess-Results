...
function computeStandings($players, $games) {
    ...
    usort($standings, function($a, $b) use ($games) {
        ...
        // Prioritize newer players (higher ID) initially before any games are played
        if ($a['score'] == 0 && $a['played'] == 0 && $b['played'] == 0) {
            if ($a['playerId'] !== $b['playerId']) return $b['playerId'] <=> $a['playerId'];
        }
        
        if ($a['rating'] !== $b['rating']) return $b['rating'] <=> $a['rating'];
        
        $h2h = headToHead($a, $b, $games);
        if ($h2h !== 0) return $h2h;
        
        // Fallback tie-break: newer player gets higher rank
